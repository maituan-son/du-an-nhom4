...
class TaiKhoan extends BaseModel{
    ...
    public function dangNhap($email){
        $sql = "SELECT A.id,A.hoTen, A.dia_chi, A.so_dien_thoai, A.email, A.matKhau, A.vai_tro_id FROM nguoi_dung A INNER JOIN vai_tro B ON A.vai_tro_id = B.id WHERE A.email = ?";
        $this->setQuery($sql);
