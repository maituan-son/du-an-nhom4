...
//            die();
            ...
            $hoa_don_id = $this->gioHang->hoaDon($ngay_dat_hang, $tong,$tinh_trang, $nguoi_dung_id);

            foreach ($_SESSION['san_pham'] as $v){
                $this->gioHang->chiTietHoaDon($_SESSION['nguoi_dung']->id,$v['id_sp'],$v['so_luong'],$v['don_gia'],$hoa_don_id);
            }
...
//        die();
        ...
        // Bình luận
        $binh_luan = $this->gioHang->loadAllBl('binh_luan' , 0 , 3);
        $this->render('client.chiTietSP',compact('load_one','binh_luan'));
    }
    // Bình luận
    public function binhLuanPost(){
        if(isset($_POST['binhLuan'])){
//            $ten_binh_luan = $_POST['name'];
//            $id_nguoi_dung = $_POST['id_nd'];
            $binh_luan= $_POST['binh_luan'];
            $san_pham_id = $_POST['san_pham_id'];
            $this->gioHang->binhLuan($binh_luan,$san_pham_id);
//            chua su ly dược phần header không chuyển sang được trang chi tiết san phẩm
            header('location: '.url(''));
        }
    }
